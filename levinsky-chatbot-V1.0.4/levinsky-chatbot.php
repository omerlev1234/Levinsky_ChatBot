...
class LevinskyChatbotAllInOne {
  ...
  private function defaults() {
    return [
      ...
      'error_notification_sender_email' => '',
    ];
  }
  ...
  public function sanitize_settings($input) {
    ...
    $sender_email = isset($input['error_notification_sender_email']) ? sanitize_email((string)$input['error_notification_sender_email']) : '';
    $out['error_notification_sender_email'] = ($sender_email && is_email($sender_email)) ? strtolower($sender_email) : '';

    return $out;
  }

  /**
   * Sanitizes a list of notification email addresses from the admin settings form.
   *
   * Accepts either a textarea string (one email per line) or an array and
   * returns a unique, lowercased list containing only syntactically valid
   * email addresses.
   */
      ...
      <form method="post" action="options.php">
        ...
        <table class="form-table" role="presentation">
          ...
          <tr>
            ...
            <td>
              Sender email:<br>
              <input type="email" style="width:420px;" name="<?php echo esc_attr($opt); ?>[error_notification_sender_email]" value="<?php echo esc_attr(isset($cfg['error_notification_sender_email']) ? $cfg['error_notification_sender_email'] : ''); ?>" placeholder="<?php echo esc_attr($this->get_notification_from_email()); ?>">
              <p class="description">Leave empty to use the WordPress admin email. When Mailjet sends these emails, this sender should be a verified sender address or belong to a verified sender domain in Mailjet.</p>
              <br>
              Recipient emails:<br>
...
PROMPT;
  ...
  private function get_notification_from_email() {
    $cfg = $this->get_settings();
    $configured_email = isset($cfg['error_notification_sender_email']) ? sanitize_email((string)$cfg['error_notification_sender_email']) : '';
    if ($configured_email && is_email($configured_email)) {
